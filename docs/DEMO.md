...
- `demo.cast` records a fresh clone entering the Nix development environment and starting
  the demo server.
...
Record a clean checkout at 100 columns by 24 rows:

```console
asciinema record --window-size 100x24 --idle-time-limit 1 docs/demo.cast
```
