...
is public credential material and one-way token digests.  Nothing in any flow gives the server
a private key, so no bug in the server package can leak one --- which is not a claim about how
carefully it was written, but a property of where the keys live: the authenticator generates
the key pair and keeps the private half, and the only things the relying party ever receives
are a public key and signatures over challenges it chose itself.
...
- *A person's passkey* is held by a WebAuthn authenticator --- a secure element, a TPM, a
  ...
  it is protected is a property of *that* authenticator, not of this project: WebAuthn permits
  hardware, software, platform, roaming and synced credentials, and a backup-eligible
  credential may be synchronized or recovered by its provider (Apple's passkeys, for instance,
  sync through end-to-end-encrypted iCloud Keychain).  What is universal is that the provider
  is not an identity provider for your application and never hands LocalWebAuthn a key ---
  =@localwebauthn/browser= touches no key material at all, because the browser performs the
  ceremony.
...
*A stable API is not a production track record.* The current release is =2.2.0=, and LocalWebAuthn
...
- [[./packages/client][=@localwebauthn/client=]] --- the software authenticator, DPoP proofs
  ...
  point, =@localwebauthn/client/file-key=.  Only for deployments issuing API credentials.
