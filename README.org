...
*LocalWebAuthn never possesses private key material.*  Not on the server, not in either
client package, not in transit, not in a log, not in a backup.  There is no moment in any
flow at which a private key passes through code in this repository --- which is why no bug in
this repository can leak one.

That is not a claim about how carefully this code was written.  It is a property of where the
keys live: the authenticator generates the key pair and keeps the private half, and the only
things the relying party ever receives are a public key and signatures over challenges it
chose itself.
...
Private keys do exist.  They live in exactly two places, neither of which is code in this
project:

- *A person's passkey* lives in the platform's own keystore --- a secure element, a TPM, a
  hardware key, or a password manager the user already trusts with everything else.  For
  device-bound credentials the key is non-exportable in hardware.  For synced platform
  passkeys the provider protects it with the user's account and device unlock, and is still
  not an identity provider for your application: it never hands LocalWebAuthn a key.
  =@localwebauthn/browser= never touches key material at all --- the browser performs the
  ceremony.
- *A script's Passkey* lives in the one file the operator provisioned: a =.env=, or a
  reference to a platform keystore.  =@localwebauthn/client= reads it and keeps it in process
  memory, and never writes it anywhere.  The mint page shows it exactly once and the server
  keeps only the public half, so there is one copy, in one place, whose custody is a decision
  you made rather than a default you inherited.
